import { CollectionDetails, Task } from "@/types/api";

const getCollectionDetails = jest.fn(
  async (pid: string): Promise<CollectionDetails> => {
    return {
      pid,
...
export const getCollectionDetailsBulk = jest.fn(
  async (pids: string[]): Promise<Record<string, CollectionDetails>> => {
